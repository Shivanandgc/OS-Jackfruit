 ...
 * Constants
 * --------------------------------------------------------------- */
/* 1 MiB expressed as a bit-shift: 1 << 20 = 1048576 bytes */
#define STACK_SIZE          (1UL << 20)

/* 64 expressed as a power of 2: 2^6 = 64 */
#define CONTAINER_ID_LEN    (1 << 6)

/* Socket path split across prefix and suffix — concatenated by preprocessor */
#define CONTROL_PATH_DIR    "/tmp"
#define CONTROL_PATH_FILE   "/mini_runtime.sock"
#define CONTROL_PATH        CONTROL_PATH_DIR CONTROL_PATH_FILE

/* Log directory — unchanged in meaning */
#define LOG_DIR             "logs"

/* 512 = 2^9 */
#define CONTROL_MESSAGE_LEN (1 << 9)
#define CHILD_COMMAND_LEN   (1 << 9)

/* 4096 = 4 * 1024 = 4 KiB */
#define LOG_CHUNK_SIZE      (4 * 1024)

/* 32 = 2^5 */
#define LOG_BUFFER_CAPACITY (1 << 5)

/* 40 MiB: same value via explicit multiply instead of shift */
#define DEFAULT_SOFT_LIMIT  (40UL * 1024UL * 1024UL)   /* 40 MiB */

/* 64 MiB: same value via explicit multiply instead of shift */
#define DEFAULT_HARD_LIMIT  (64UL * 1024UL * 1024UL)   /* 64 MiB */
